...
asUINT __cdecl CScriptArray_opForBegin(const CScriptArray*)
{
	return 0;
}

bool __cdecl CScriptArray_opForEnd(asUINT iter, const CScriptArray*arr)
{
...
asUINT __cdecl CScriptArray_opForNext(asUINT iter, const CScriptArray*)
{
	return iter + 1;
}

asUINT __cdecl CScriptArray_opForValue1(asUINT iter, const CScriptArray*)
{
	return iter;
}
...
static void RegisterScriptArray_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_TEMPLATE_CALLBACK, "bool f(int&in, bool&out)", asFUNCTION(ScriptArrayTemplateCallback), asCALL_STDCALL); assert( r >= 0 );

	// Templates receive the object type as the first parameter. To the script writer this is hidden
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in)", asFUNCTIONPR(CScriptArray::Create, (asITypeInfo*), CScriptArray*), asCALL_STDCALL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in, uint length) explicit", asFUNCTIONPR(CScriptArray::Create, (asITypeInfo*, asUINT), CScriptArray*), asCALL_STDCALL); assert( r >= 0 );
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_FACTORY, "array<T>@ f(int&in, uint length, const T &in value)", asFUNCTIONPR(CScriptArray::Create, (asITypeInfo*, asUINT, void *), CScriptArray*), asCALL_STDCALL); assert( r >= 0 );

	// Register the factory that will be used for initialization lists
	r = engine->RegisterObjectBehaviour("array<T>", asBEHAVE_LIST_FACTORY, "array<T>@ f(int&in type, int&in list) {repeat T}", asFUNCTIONPR(CScriptArray::Create, (asITypeInfo*, void*), CScriptArray*), asCALL_STDCALL); assert( r >= 0 );
	...
	// Support for foreach
	r = engine->RegisterObjectMethod("array<T>", "uint opForBegin() const", asFUNCTIONPRC(CScriptArray_opForBegin, (const CScriptArray *), asUINT, __cdecl), asCALL_CDECL_OBJLAST); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "bool opForEnd(uint) const", asFUNCTIONPRC(CScriptArray_opForEnd, (asUINT, const CScriptArray*), bool, __cdecl), asCALL_CDECL_OBJLAST); assert(r >= 0);
	r = engine->RegisterObjectMethod("array<T>", "uint opForNext(uint) const", asFUNCTIONPRC(CScriptArray_opForNext, (asUINT, const CScriptArray*), asUINT, __cdecl), asCALL_CDECL_OBJLAST); assert(r >= 0);
	...
	r = engine->RegisterObjectMethod("array<T>", "uint opForValue1(uint index) const", asFUNCTIONPRC(CScriptArray_opForValue1, (asUINT, const CScriptArray*), asUINT, __cdecl), asCALL_CDECL_OBJLAST); assert(r >= 0);
	...
	r = engine->RegisterObjectMethod("array<T>", "bool get_IsEmpty() const property", asMETHOD(CScriptArray, IsEmpty), asCALL_THISCALL); assert( r >= 0 );
